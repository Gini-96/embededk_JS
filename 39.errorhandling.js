...
// 에러 발생 가능한 코드들이 위치하는 블럭
try {
    v = 10;
    let v;
// 에러 처리하는 코드들이 위치하는 블럭
// 발생한 에러객체가 전달
} catch (err) {
    console.error('에러메세지 : '+ err.message);
    console.log('발생한 에러 처리');
} finally {
    console.log('에러발생 여부와 상관 없이 수행');
}
...
// 에러 전파
// thirdFunc에서 발생한 에러가 secondFunc를 호출한 firstFunc에서 처리됨
const firstFunc = function() {
    // firstFunc에서 직접 에러 처리
    try {
        secondFunc();
    } catch(err) {
